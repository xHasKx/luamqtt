...
-- @param args: a table with fields:
--		uri			- [required, string]:
--			MQTT broker uri to connect, in format host:port" or "host", port will be selected automatically
--			using 1883 port for plain or 8883 for secure network connections (provided by luasec)
...
--		secure		- [optional, boolean or table]:
--			use secure network connection, provided by luasec lua module; false by default
--			set to true to select default params: { mode="client", protocol="tlsv1_2", verify="none", options="all" }
--			or set to luasec-compatible table, for example with params cafile="...", certificate="...", key="..."
--		will		- [optional, table]:
...
--		reconnect	- [optional, boolean or number]:
--			force created MQTT client to reconnect on connection close, false by default
--			set to number value to provide reconnect timeout in seconds
--			it's not recommended to use value < 3
--		connector	- [optional, table]:
--			connector table to open and send/receive packets over network connection, default is require("mqtt.luasocket")
--			if secure argument is set - using require("mqtt.luasocket_ssl") connector to use luasec lua module
-- @return MQTT client instance table
function client_mt:__init(args)
	if not library_version then
		library_version = require("mqtt")._VERSION
	end
	...
	if not a.id then
		...
		a.id = str_format("luamqtt-v%s-%07x", str_gsub(library_version, "[^%d]", "-"), math_random(1, 0xFFFFFFF))
	end
...
-- Add function as handler of given event: cl:on(event, func)
-- OR add given functions as event handlers: cl:on{ event1 = func1, event2 = func2 }
...
--- Subscribe to specified topic. Returns the SUBSCRIBE packet id
-- @tparam string topic [required] topic to subscribe
-- @tparam number qos [optional] qos level for subscription
-- @tparam function callback [optional] callback function to be called when subscription will be created
...
function client_mt:subscribe(topic, qos, callback)
	...
	local subscribe = make_packet4(pargs)
...
function client_mt:unsubscribe(topic, callback)
	...
	self:_assign_packet_id(pargs)
	local packet_id = pargs.packet_id
	local unsubscribe = make_packet4(pargs)
...
--- Publish message to broker. For QoS > 0 messages returns the packet id of sent message
-- @tparam table args [required] arguments
...
function client_mt:publish(args)
	...
	local publish = make_packet4(args)
...
function client_mt:acknowledge(msg)
	...
	if msg.qos == 1 then
		...
		local puback = make_packet4{type=packet_type.PUBACK, packet_id=packet_id}
	...
	elseif msg.qos == 2 then
		...
		local pubrec = make_packet4{type=packet_type.PUBREC, packet_id=packet_id}
...
function client_mt:acknowledge_pubrel(packet_id)
	...
	local pubrel = make_packet4{type=packet_type.PUBREL, packet_id=packet_id}
...
function client_mt:acknowledge_pubcomp(packet_id)
	...
	local pubcomp = make_packet4{type=packet_type.PUBCOMP, packet_id=packet_id}
...
--- Immediately close established network connection
-- @tparam string reason [optional] reasong string for connection closing
...
function client_mt:_ioloop_iteration()
	...
	if conn then
		...
		if not ok then
			err = "failed to resume receive packet coroutine: "..err
			self:handle("error", err, self)
			self:close_connection("error")
			return false, err
		end
...
function client_mt:_apply_network_timeout()
	...
	if conn then
		...
		if loop then
			...
			self.args.connector.settimeout(conn, loop.opts.timeout)
...
function client_mt:_receive_packet()
	...
	-- parse packet
	local packet, err = parse_packet4(conn.recv_func)
	if not packet then
		return false, err
	end
	self.comm_time = os_time()
...
-- See more about ... args in client_mt:__init() documentation
